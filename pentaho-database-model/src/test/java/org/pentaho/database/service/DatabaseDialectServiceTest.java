...
public class DatabaseDialectServiceTest {
  ...
  @Test
  public void testValidateJdbcDriverClass() {
    DatabaseDialectService dialectService = new DatabaseDialectService();
  ...
  public void testGetDatabaseTypes() {
    ...
    Assert.assertTrue("When running as a test case, there should minimally be the generic type in this list.", foundGeneric);
    dialectService = new DatabaseDialectService(false);
    dbTypes = dialectService.getDatabaseTypes();
    Assert.assertNotNull( "dbTypes should not come back null.", dbTypes );
    Assert.assertTrue( "There should minimally be found 15 types here based on the classes in org.pentaho.database.dialect", (dbTypes.size() >= 15) );    
  }
  
  @Test
  public void testGetDatabaseDialects() {
    DatabaseDialectService dialectService = new DatabaseDialectService();
    ...
    Assert.assertTrue("When running as a test case, there should minimally be the generic dialect in this list.", foundGeneric);
    dialectService = new DatabaseDialectService(false);
